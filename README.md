...
Trên Colab, mở chính file `CrackSpot.ipynb`; URL repository và chế độ clone/cài dependency tự động đã được cấu hình sẵn. Chỉ cần giữ `DO_DOWNLOAD_SDNET2018=True` và `DATASET_DOWNLOAD_MODE="auto"`, sau đó chạy từ trên xuống. Notebook clone repository để nhận locked manifests vào `/content/CrackSpot`; code chạy vẫn nằm trọn trong notebook. Notebook thử archive chính thức trước và tự chuyển sang Kaggle nếu DigitalCommons trả 403/timeout. Archive chính thức được xác minh MD5; nguồn dự phòng hoặc ZIP do người dùng cung cấp được đối chiếu SHA-256 của đủ 56.088 ảnh theo locked manifests trước khi train. `INSTALL_DEPENDENCIES=None` nghĩa là tự cài trên Colab và không tự cài ở local.
